...
export async function GET() {
  ...
  try {
    ...
    const nullChainData = data.result.map((protocol) => ({
      protocol_name: protocol.protocol_name,
      value:
        protocol.result.find((item) => item.chain_id === null)?.value_usd || 0,
    }));

    // Calculate total value
    const totalValue = nullChainData.reduce((sum, item) => sum + item.value, 0);
